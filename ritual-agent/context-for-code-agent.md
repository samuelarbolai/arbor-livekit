...
- **2026-06-18 — Demo Call: grado-driven desidentificación skip (`flows/demo-call/prompts/demo-call-prompt.ts`):** `grado_de_identificacion` (low|medium|high, judged at end of Phase 5b) is now the back-half branch driver and REPLACED `fit_state`. **high** → full desid arc (Phase 6 full → 7 → 8); **low/medium** → short Phase 6 acknowledgment block, SKIP Phases 7 & 8, Phase 9 with the mantra-claim sentence reframed (doc+promise visuals still play for everyone); **low** also APPENDS the referral Phases 16–17 after the close. Phase 8.5 was rewritten from an evaluation/`fit_state`-setter into a warm fit acknowledgment. The admission-test scarcity pause was RELOCATED from Phase 8.5 to the end of Phase 5b (the genuine evaluation moment). New `<how-grado-branches>` block + `buildCurrentPhaseBlock`/`<how-you-navigate>` updated to permit the branch skips. `fit_state` no longer set on the call. Branch logic asserted by `prompts/demo-call-prompt.test.ts`. Lockstep with the human-copilot script Doc (block-level `[CONDITION:]` in samwise-app `script-pane.tsx`) — see `samwise-demo-call-agent` + `samwise-session-copilot` skills.
