...
- **Gemini `recordRelapse` fragility (still open).** 3 of 8 LLM-driven tests fail intermittently because Gemini-3-flash-preview is less decisive about firing `recordRelapse` than OpenAI is. Behavior, not architecture. Accepted as ship-now/iterate-later — revisit by either tuning the prompt's relapse phrasing or upgrading the model. Tracking here so it doesn't get lost.

### Tech-debt notes 📌
- **`pnpm.overrides.openai = "6.8.1"`** in `package.json` — workaround for `@livekit/agents@1.3.2` passing `apiKey: ""` (intentional; runtime swap to a generated LK access token) which `openai@6.36.0` rejects at construction. Drop the override once `@livekit/agents` ships a fix that passes `undefined` instead of `""`. Auth on the wire flows through LiveKit Inference using `LIVEKIT_API_KEY/SECRET`; OpenAI credentials are never sent. Without this pin, both `pnpm dev`/`pnpm start` AND `pnpm test` would fail at `inference.LLM` construction time.
